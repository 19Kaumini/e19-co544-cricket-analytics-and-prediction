...
class Duranz(Vizuals):
    ...
    def scorecard(self, team=1):
        ...
        extras_played = [
            x
            for x in ["-", "legbyes", "byes", "noballs"]
            if team["extra_type"].isin([x]).any()
        ]
        balls_played = (
            team.groupby(["extra_type", "batter"])
            .count()
            .loc[extras_played]
            .sum(level="batter")["over"]
        )
        ...
        def boundaries(df):
            ...
            sixes = (
                boundary.loc[6].iloc[:, 0].to_frame("6's")
                if 6 in boundary.index
                else pd.DataFrame({"batter": [], "6's": []})
            )
            boundaries_df = fours.merge(sixes, how="outer", on="batter")
            return boundaries_df

        def wicket(df):
            result = df.query("wicket_type != 0")[
                ["batter", "wicket_type", "fielder", "bowler"]
            ]
            return result

        temp_result = pd.DataFrame(
            [batsman_score, balls_played], index=["runs", "balls"]
        ).T.reindex(batting_order(team))
